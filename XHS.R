...
### -------- create new listing ------------------------
new_season <- "26"
mastersku <- openxlsx::read.xlsx(rownames(file.info(list.files(path = "../../TWK 2020 share/", pattern = "1-MasterSKU-All-Product-", full.names = TRUE)) %>% filter(mtime == max(mtime))), sheet = "MasterFile", startRow = 4, fillMergedCells = T) %>% mutate(SPU = paste(Category.SKU, Print.SKU, sep = "-")) %>% `row.names<-`(toupper(.[, "MSKU"]))
catprint <- openxlsx::read.xlsx(list.files(path = "../../TWK 2020 share/", pattern = "1-MasterSKU_CatPrintsFactory.xlsx", full.names = T), sheet = "SKU - product category", fillMergedCells = T) %>% filter(!duplicated(Category_SKU), !is.na(Category_SKU)) %>% `row.names<-`(.[, "Category_SKU"])
image <- read.table("../woo/ImageSrc.txt", as.is = T, header = T, sep = "\t") %>% filter(!is.na(SKU), !is.na(Images), SKU != "") %>% `row.names<-`(.[, "SKU"])
...
##### Add a new category
categories_exclude <- c("TPL", "TPS", "TTL", "TSWV", "TSWU", "TSAU", "PTSU", "PTAU", "PJWU", "CTDU", "TPLU", "TPSU", "WPFU", "WJAU", "WJSU", "WPSU", "PWJ", "TTSU", "TTLU", "PLAU", "KSCU", "TICKET", "Fall_Legging", "SSW", "SSS", "SKT", "SKB-INSOL", "MWPS", "MWPF", "MSXP", "MSKB", "MKMT", "DEAL-MISC-KCS", "GUB-BOX", "GUB-AST", "FMG", "FVMU", "AWP", "AAP")
...
products_description_cat <- rbind(products_description_cat, data.frame(cat = unique(item_new$Item.Category.SKU)[!(unique(item_new$Item.Category.SKU) %in% products_description_cat$cat)]) %>% mutate(Product.Name = paste("Jan & Jul", catprint[cat, "Chinese_Name"]), Description = "", Categories = catprint[cat, "Parent_Category.(Chinese)"], Option1.Name = "Size"))
write.xlsx(products_description_cat, file = "../XHS/products_description_categories.xlsx", row.names = F)
...
new_listing <- data.frame(SKU = item_new$Name) %>% 
  ...
  filter(status == "Active", !is.na(Price), !(SKU %in% toupper(products_XHS$SKU)), cat %in% products_description_cat$cat, SPU %in% products_description$SPU)  %>% 
