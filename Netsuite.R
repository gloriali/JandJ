...
netsuite_so <- clover_so %>% filter(Item.SKU != "", Order.Payment.State == "Paid") %>% mutate(Order.date = format(as.Date(gsub(" .*", "", Line.Item.Date), "%d-%b-%Y"), "%m/%d/%Y")) %>% group_by(Order.date) %>% 
  mutate(Account = "1150 First Data / Clover Clearing", MEMO = "Clover sales", ID = data.table::rleid(Order.ID), REF.ID = paste0("Clover", format(as.Date(Order.date, "%m/%d/%Y"), "%Y%m%d"), "-", sprintf("%02d", ID)), Order.Type = "JJR OFFLINE", Department = "Retail : Store Richmond", Warehouse = "WH-Richmond", Quantity = 1, Price.level = "Custom", Rate = Item.Revenue, Amount = Item.Total, Coupon.Discount = Total.Discount + Order.Discount.Proportion, Coupon.Code = gsub("NA", "", gsub(" -.*","", paste0(Order.Discounts, Discounts))), Tax.Code = ifelse(Item.Tax.Rate == 0.05, "CA-BC-GST", ifelse(Item.Tax.Rate == 0.12, "CA-BC-TAX", ""))) %>% 
  select(Account, Order.date, REF.ID, Order.Type, Department, Warehouse, MEMO, Order.ID, Item.SKU, Quantity, Price.level, Rate, Amount, Coupon.Discount, Coupon.Code, Tax.Code, Tax.Amount)
...
netsuite_so <- square_so %>% filter(Item != "", Location == "Surrey") %>% mutate(Order.date = format(as.Date(Date, "%Y-%m-%d"), "%m/%d/%Y")) %>% group_by(Order.date) %>% 
  mutate(Account = "1152 Square Clearing Account", MEMO = "Square sales", Order.ID = Transaction.ID, Item.SKU = Item, ID = data.table::rleid(Order.ID), REF.ID = paste0("Square", format(as.Date(Order.date, "%m/%d/%Y"), "%Y%m%d"), "-", sprintf("%02d", ID)), Order.Type = "JJR OFFLINE", Department = "Retail : Store Surrey", Warehouse = "WH-SURREY", Quantity = Qty, Price.level = "Custom", Gross.Sales = as.numeric(gsub("\\$", "", Gross.Sales)), Net.Sales = as.numeric(gsub("\\$", "", Net.Sales)), Discounts = as.numeric(gsub("\\$", "", Discounts)), Tax = as.numeric(gsub("\\$", "", Tax)), Rate = round(Gross.Sales/Qty, 2), Amount = Net.Sales, Coupon.Discount = Discounts, Coupon.Code = "", Tax.Code = ifelse(round(Tax/Net.Sales, 2) == 0.05, "CA-BC-GST", ifelse(round(Tax/Net.Sales, 2) == 0.12, "CA-BC-TAX", "")), Tax.Amount = Tax) %>% 
  select(Account, Order.date, REF.ID, Order.Type, Department, Warehouse, MEMO, Order.ID, Item.SKU, Quantity, Price.level, Rate, Amount, Coupon.Discount, Coupon.Code, Tax.Code, Tax.Amount)
