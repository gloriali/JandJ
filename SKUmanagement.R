...
deals <- xoro %>% mutate(lastmonth = xoro_lastmonth[Item., "ATS"]) %>% filter(cat_print %in% size_limited$cat_print & !grepl(new_season, seasons) & ATS > qty_offline & lastmonth - ATS < qty_sold & lastmonth - ATS > 0) %>% 
