...
for i in POn:
    ...
    CA_subtotal = pd.DataFrame({'POplan': [CA_detail['POplan'].sum()], 'Remain_Pc': [CA_detail['POplan'].sum()], 'Remain_percent': '' if all(s == '' or str(s).isspace() for s in CA_POplan) else format(1,'.1%')})
    with pd.ExcelWriter("../PO/" + season + " China to Global Shipment Tracking.xlsx", if_sheet_exists = 'overlay', mode = 'a') as writer:
    ...
    UK_subtotal = pd.DataFrame({'POplan': [UK_detail['POplan'].sum()], 'Remain_Pc': [UK_detail['POplan'].sum()], 'Remain_percent': '' if all(s == '' or str(s).isspace() for s in CA_POplan) else format(1,'.1%')})
    with pd.ExcelWriter("../PO/" + season + " China to Global Shipment Tracking.xlsx", if_sheet_exists = 'overlay', mode = 'a') as writer:
    ...
    DE_subtotal = pd.DataFrame({'POplan': [DE_detail['POplan'].sum()], 'Remain_Pc': [DE_detail['POplan'].sum()], 'Remain_percent': '' if all(s == '' or str(s).isspace() for s in CA_POplan) else format(1,'.1%')})
    with pd.ExcelWriter("../PO/" + season + " China to Global Shipment Tracking.xlsx", if_sheet_exists = 'overlay', mode = 'a') as writer:
    ...
    CN_subtotal = pd.DataFrame({'POplan': [CN_detail['POplan'].sum()], 'Remain_Pc': [CN_detail['POplan'].sum()], 'Remain_percent': '' if all(s == '' or str(s).isspace() for s in CA_POplan) else format(1,'.1%')})
    with pd.ExcelWriter("../PO/" + season + " China to Global Shipment Tracking.xlsx", if_sheet_exists = 'overlay', mode = 'a') as writer:
    ...
    WS_subtotal = pd.DataFrame({'POplan': [WS_detail['POplan'].sum()], 'Remain_Pc': [WS_detail['POplan'].sum()], 'Remain_percent': '' if all(s == '' or str(s).isspace() for s in CA_POplan) else format(1,'.1%')})
    with pd.ExcelWriter("../PO/" + season + " China to Global Shipment Tracking.xlsx", if_sheet_exists = 'overlay', mode = 'a') as writer:
