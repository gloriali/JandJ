...
netsuite_so <- XHS_so %>% filter(Financial.Status == "paid", Shipping.Country != "中国") %>% mutate(Order.Date = format(as.Date(gsub(" .*", "", Created.At), "%Y-%m-%d"), "%m/%d/%Y"), Order.ID = paste0("XHS", Shipping.Country, "-", Order.No), Shipping.Name = ifelse(Delivery.Method == "pickup", Pickup.Person.Name, paste(Shipping.First.Name, Shipping.Last.Name))) %>% 
  mutate(Customer.ID = "", Order.Type.Code = paste("XHS", Shipping.Country), Ref.Number = Order.ID,	Date = format(Sys.Date(), "%m/%d/%Y"), Sales.Effective.Date = Order.Date, Ship.By.Date = as.Date(Order.Date, "%m/%d/%Y") + 1,	Price.Level = "Custom",	SKU = LineItem.SKU,	Unit.Price = LineItem.UnitPrice, QTY = LineItem.Quantity, Amount = LineItem.Total, Tax.Code = "", Coupon_Discount = LineItem.Total.Discount, Order.Coupon.Code = LineItem.Promotion.Detail, Shipping.cost = Shipping, Shipping.Tax.Code = "", Woocommerce.Order.Total = Total, Marketplace.Shipping.Method = Delivery.Method, Commit = "Available QTY",	Marketplace.PAYMENT.METHOD = Payment.Method, Payment.Option = ifelse(Payment.Method == "Stripe", "Stripe CAD Retail", "AlphaPay"), Terms = "CIA", To.Be.Emailed = "No", Shipping.Postal.Code = Shipping.Zip, Billing.Name = Shipping.Name, Billing.Address1 = Shipping.Address1, Billing.Address2 = Shipping.Address2, Billing.City = Shipping.City, Billing.Province = Shipping.Province, Billing.Country = Shipping.Country, Billing.Postal.Code = Shipping.Zip, Department = "Retail : Marketplace : XiaoHongShu", Class = paste("FBM :", Shipping.Country),	Warehouse = "WH-SURREY", Recipient = Shipping.Name, Recipient.Email = Email, Recipient.Phone = Phone, Shipstation.Carrier = "Canada Post",	Shipstation.Shipping.Service = "Expedited Parcel", SHIPSTATION.SERVICE = "Standard", Order.Issue = "", Memo = "") %>%
