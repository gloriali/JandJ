...
# ------------- upload Clover SO to NS, correct Clover inventory and update price: weekly ---------------------------
...
Surrey_employee <- c("Garman", "Iris", "Sophie")
...
if(nrow(customer %>% filter(grepl("\\|", Email.Address)))){
  View(customer %>% filter(grepl("\\|", Email.Address)))
}else{print("Customer list is good.")}
...
netsuite_so <- clover_so %>% filter(Item.SKU != "", Order.Payment.State == "Paid", Refunded == "false", Exchanged == "false", !grepl("XHS", Order.Discounts), !grepl("Surrey", Order.Discounts)) %>% mutate(Order.date = format(as.Date(gsub(" .*", "", Line.Item.Date), "%d-%b-%Y"), "%m/%d/%Y")) %>% group_by(Order.date) %>% 
  mutate(Payment.Option = ifelse(Tender == "Cash", "Cash", ifelse(grepl("WeChat", Tender), "AlphaPay", "Clover")), Class = "FBM : CA", MEMO = "Clover sales", Customer = ifelse(Order.Employee.Name %in% Surrey_employee, "55 JJR SHOPS", "54 JJR SHOPR"), ID = data.table::rleid(Order.ID), REF.ID = ifelse(Order.Employee.Name %in% Surrey_employee, paste0("CLS", format(as.Date(Order.date, "%m/%d/%Y"), "%y%m%d"), "-", sprintf("%02d", ID)), paste0("CLR", format(as.Date(Order.date, "%m/%d/%Y"), "%y%m%d"), "-", sprintf("%02d", ID))), Order.Type = ifelse(Order.Employee.Name %in% Surrey_employee, "JJR SHOPS", "JJR SHOPR"), Department = ifelse(Order.Employee.Name %in% Surrey_employee, "Retail : Store Surrey", "Retail : Store Richmond"), Warehouse = ifelse(Order.Employee.Name %in% Surrey_employee, "WH-SURREY", "WH-RICHMOND"), Quantity = 1, Price.level = "Custom", Rate = Item.Total, Coupon.Discount = Total.Discount + Order.Discount.Proportion, Coupon.Discount = ifelse(Coupon.Discount == 0, NA, Coupon.Discount), Coupon.Code = gsub("NA", "", gsub(" -.*","", paste0(Order.Discounts, Discounts))), Tax.Code = ifelse(Item.Tax.Rate == 0.05, "CA-BC-GST", ifelse(Item.Tax.Rate == 0.12, "CA-BC-TAX", "")), SHIPPING.CARRIER = "", SHIPPING.METHOD = "", SHIPPING.COST = 0) %>% 
...
# upload to NS
update_INV <- T
adjust_inventory <- clover_so %>% filter(Order.Employee.Name %in% Surrey_employee) %>% group_by(Item.SKU) %>% summarise(Quantity = n()) %>% as.data.frame() %>% `row.names<-`(.[, "Item.SKU"])
...
price <- woo %>% mutate(cat = gsub("-.*", "", SKU)) %>% group_by(cat) %>% summarise(Price = max(Sale.price, na.rm = T)) %>% as.data.frame() %>% `row.names<-`(toupper(.[, "cat"])) 
...
if(update_INV){
  print("Update INV")
  clover_item <- wb_to_df(clover, "Items") %>% mutate(Quantity = ifelse(SKU %in% rownames(adjust_inventory), Quantity + adjust_inventory[SKU, "Quantity"], Quantity)) %>% filter(Name != "") %>% 
    mutate(cat = gsub("-.*", "", Name), Price = ifelse(Name %in% woo$SKU, woo[Name, "Sale.price"], price[cat, "Price"]), `Alternate Name` = woo[Name, "Name"], `Tax Rates` = ifelse(cat %in% PST, "GST+PST", "GST")) 
}else{
...
global_discount <- 0.9
clover_item <- clover_item %>% mutate(Price = ifelse(Name %in% woo$SKU, ifelse(woo[Name, "Sale.price"] < woo[Name, "Regular.price"]*global_discount, woo[Name, "Sale.price"], round(woo[Name, "Regular.price"]*global_discount, 2)), round(price[cat, "Price"] * global_discount, 2)), `Price Type` = ifelse(is.na(Price), "Variable", "Fixed")) 
## clearance price
clover_item <- clover_item %>% regex_left_join(clearance, by = c("Name" = "Item")) %>% mutate(Price = coalesce(ifelse(Sales > 1, Sales, round(woo[Name, "Regular.price"]*(1-Sales), 2)), Price), Price = ifelse((Name %in% woo$SKU) & (Price > woo[Name, "Sale.price"]), woo[Name, "Sale.price"], Price), `Price Type` = ifelse(is.na(Price), "Variable", "Fixed")) %>% select(all_of(names(clover_item))) %>% distinct(Name, .keep_all = T) %>% filter(!is.na(Name)) %>% select(-cat)
...
write.csv(point, file = paste0("../yotpo/", format(Sys.Date(), "%m%d%Y"), "-yotpo.csv"), row.names = F, fileEncoding = "UTF-8")
# upload to Yotpo
...
write_xlsx(products_upload, file = paste0("../XHS/products_upload-", format(Sys.Date(), "%Y-%m-%d"), ".xlsx"), na = NULL)
...
netsuite_item_S <- netsuite_item %>% filter(Inventory.Warehouse == "WH-SURREY") %>% `row.names<-`(.[, "Name"])
season <- "26F"
request <- c("AJA", "AJC", "AJM", "BCV", "BRC", "BHT", "BHA", "BTA", "BST", "BSL", "BTB", "BTL", "MBTL", "BTT", "BST", "FAN", "FHA", "FHB", "FHS", "AJR", "FJR", "FJZ", "FJC", "FJM", "FPM", "FMR", "FGR", "FSM", "FVM", "IHT", "IAJ", "IAB", "IPC", "ICP", "IPS", "ISJ", "ISS", "ISB", "JCC", "PCC", "TSW", "PJW", "PLW", "KEH", "KHB", "KHP", "KMN", "KMT", "KGL", "LAB", "LAN", "LAV", "LBT", "LBP", "LCP", "LCT", "SLC", "SLM", "SUK", "SKG", "SKB", "SKX", "SMC", "SMF", "SWS", "MSWS", "WJA", "WJT", "WPF", "WPS", "WSF", "WSS", "WBF", "WBS", "WJO", "WPO", "WHO", "WHR", "WJY", "WPY", "WGF", "WGS", "WMT", "WRM", "XBY", "XBK", "XBM", "XLB", "XPC") # categories to restock for FW
...
order <- data.frame(Date = format(Sys.Date(), "%m/%d/%Y"), TO.TYPE = "Surrey-Richmond", SEASON = season, FROM.WAREHOUSE = "WH-SURREY", TO.WAREHOUSE = "WH-RICHMOND", REF.NO = paste0("TO-S2R", format(Sys.Date(), "%y%m%d")), Memo = "Richmond Refill", ORDER.PLACED.BY = "Gloria Li", ITEM = (clover_item %>% filter(Quantity < n1))$Name) %>% 
  mutate(CAT = netsuite_item_S[ITEM, "Item.Category.SKU"], SIZE = netsuite_item_S[ITEM, "Item.Size"], NAME = netsuite_item_S[ITEM, "Display.Name"], Seasons = netsuite_item_S[ITEM, "Item.SKU.Seasons"], Quantity = ifelse(CAT %in% cat & SIZE %in% size & grepl(season, Seasons), n1 - clover_item[ITEM, "Quantity"], n - clover_item[ITEM, "Quantity"])) %>% filter(ITEM %in% netsuite_item_S$Name, netsuite_item_S[ITEM, "Warehouse.Available"] > n_S) %>% filter(Quantity > 0) %>% mutate(cat = gsub("-.*", "", ITEM), size = gsub("\\w+-\\w+-", "", ITEM))
order <- order %>% filter(cat %in% request) %>% select(-CAT, -SIZE)
...
f <- "../../With Mei-LAPTOP-P3O5JD1S/0 - 2019 shipments shared with Mei/Taiwan shipments/1 - Taiwan shipments/2026-08 shipment/UPC code.csv"
file <- read.csv(f, as.is = T) %>% mutate(UPC = mastersku[SKU, "UPC Active"])
write_excel_csv(file, file = f, na = "")

# ---------------- Check and update AMZPrep shipment orders ---------------
synced <- read_xlsx("../AMZPrep/SyncedOrder.xlsx", sheet = 1)
shipped <- read_xlsx("../AMZPrep/ShippedOrder.xlsx", sheet = 1)
records <- wb_load("../../TWK Order Process/01 - shipping tracking/3-AMZPrep Order Tracking - 20251223.xlsx")
records_AMZPrep <- wb_to_df(records, "Shipments", detectDates = T) %>% mutate(eShipper.Status = ifelse(Order.ID %in% synced$Extra.Note.1, "Synced", ifelse(Order.ID %in% shipped$Extra.Note.1, "Shipped", eShipper.Status)))
...
wb_save(records, file = "../../TWK Order Process/01 - shipping tracking/3-AMZPrep Order Tracking - 20251223.xlsx", overwrite = T)
...
# ---------------- Check AMZPrep inventory ---------------
netsuite_item <- read.csv(list.files(path = "../NetSuite/", pattern = paste0("Items_All_", format(Sys.Date(), "%Y%m%d"), ".csv"), full.names = T), as.is = T)
netsuite_item[netsuite_item == "" | is.na(netsuite_item)] <- 0
netsuite_item_A <- netsuite_item %>% filter(Inventory.Warehouse == "AMZPrep-CA") %>% `row.names<-`(.[, "Name"])
AMZPrep_inventory <- read_xlsx(list.files(path = "../AMZPrep/", paste0("inventory_", format(Sys.Date(), "%Y%m%d"), ".xlsx"), full.names = T), sheet = 1) 
diff <- AMZPrep_inventory %>% mutate(NS = ifelse(SKU %in% netsuite_item_A$Name, netsuite_item_A[SKU, "Warehouse.Available"], 0), diff = NS - as.numeric(Current.ATP)) %>% filter(diff != 0) %>% select(SKU, Current.ATP, NS, diff)

# -------- Analysis: monthly --------------------
new_season <- "26"   # New season contains (yr)
...
#   mr_long <- rbind(mr_long, mr_long_i)
# }
# mr_short <- mr_long %>% mutate(month = paste0("Month", sprintf("%02d", month))) %>% pivot_wider(id_cols = c(channel, category, year, annual_sales), names_from = month, values_from = monthly_ratio)
...
for(sheet in sheets){
  sales_last12month_i <- read_xlsx(RawData, sheet = sheet, cols = columns, start_row = 2, skip_empty_cols = T) 
  if(sum(grepl("janandjul.com", sales_last12month_i[[1]]))){
    ...
    sales_SKU_last12month_i <- sales_last12month_i %>% filter(!(`Sales Channel` %in% c("Summary", "Sales Channel", "All Marketplaces"))) %>% mutate_all(~ replace(., is.na(.), 0)) %>% rename_with(~ gsub("20", "Total.20", gsub("_x000D_.Total", "", .x)))
...
## SKUs to inactive
clover <- read_xlsx(list.files(path = "../Clover/", pattern = paste0("inventory", format(Sys.Date(), "%Y%m%d"), ".xlsx"), full.names = T), sheet = "Items")  %>% filter(!is.na(Name)) %>% `row.names<-`(.[, "Name"])
...
discount_method <- read_xlsx("../Analysis/JJ_discount_method.xlsx", sheet = 1, startRow = 2, rowNames = T)
...
clover <- wb_load(list.files(path = "../Clover/", pattern = paste0("inventory", format(Sys.Date(), "%Y%m%d"), ".xlsx"), full.names = T))
clover_item <- wb_to_df(clover, "Items") %>% filter(!is.na(Name), !duplicated(Name)) %>% `row.names<-`(.[, "Name"]) 
...
clover_cat_upload$Category.ID[duplicated(clover_cat_upload$Category.ID)] <- ""
clover_cat_upload <- clover_cat_upload %>% rename_with(~ gsub("\\.", " ", colnames(clover_cat_upload)))
clover_update <- wb_workbook()
for(s in clover$get_sheet_names()){clover_update <- clover_update |> wb_add_worksheet(sheet = s) |> wb_add_data(sheet = s, x = wb_to_df(clover, s))}
clover_update <- clover_update |> wb_clean_sheet(sheet = "Items") |> wb_add_data(sheet = "Items", x = clover_item_upload) |> wb_clean_sheet(sheet = "Categories") |> wb_add_data(sheet = "Categories", x = clover_cat_upload) |> wb_add_numfmt(sheet = "Tax Rates", dims = "C2:C3", numfmt = "0%")
wb_save(clover_update, file = paste0("../Clover/inventory", format(Sys.Date(), "%Y%m%d"), "-upload.xlsx"), overwrite = T)
# upload to Clover > Inventory
...
season <- "2025SS"
startRow <- 9
mastersku <- read_xlsx(list.files(path = "../../TWK 2020 share/", pattern = "1-MasterSKU-All-Product-", full.names = T), sheet = "MasterFile", startRow = 4, fillMergedCells = T) %>% `row.names<-`(.[, "MSKU"]) 
qty0 <- data.frame()
skus <- c()
for(i in c(346, 368, 387:403)){
  ...
  for(f in 1:length(file)){
    barcode <- read_xlsx(file[f], sheet = 1, startRow = startRow) %>% select(SKU, Design.Version, TOTAL.ORDER) %>% 
...
new_SKU <- data.frame(SKU = (mastersku %>% filter(Seasons == season))$MSKU) %>% 
  mutate(Description = mastersku[SKU, "Item Name"], cat = trimws(mastersku[SKU, "Category SKU"]), Cost = ifelse(SKU %in% woo$SKU, woo[SKU, "Sale.price"], price[cat, "Price"]), min.Qty = 0, UPC = gsub("\\/.*", "", gsub("\\|.*", "", mastersku[SKU, "UPC Active"])), cat_size = paste0(trimws(mastersku[SKU, "Category SKU"]), "_", trimws(mastersku[SKU, "Size"])), cat_size = ifelse(cat_size %in% rownames(dim), cat_size, gsub("[A-Z]$", "", cat_size)), length = round(dim[cat_size, "Length.in."], 2), width = round(dim[cat_size, "Width.in."], 2), height = round(dim[cat_size, "Height.in."], 2), weight = round(dim[cat_size, "Weight.lb..pc"], 2)) %>% 
...
mastersku <- read_xlsx(rownames(file.info(list.files(path = "../FBArefill/Raw Data File/", pattern = "1-MasterSKU-All-Product-", full.names = TRUE)) %>% filter(mtime == max(mtime))), sheet = "MasterFile", startRow = 4, fillMergedCells = T) %>% `row.names<-`(.[, "MSKU"])
...
season <- "26F"
folder <- stri_remove_empty(gsub(" *- *sent.*", "", gsub(paste0("\\.\\.\\/PO\\/order\\/", season, "\\/"), "", list.files(path = paste0("../PO/order/", season, "/"), pattern = "^P"))))
SeasonStart <- read.csv("../PO/SeasonStart.csv", as.is = T) %>% mutate(receive_date = format(as.Date(arrival_date, format = "%Y-%m-%d"), "%m/%d/%Y")) %>% `row.names<-`(.[, "category"]) 
...
for(f in folder){
  ...
  PO <- read_xlsx(file, sheet = 1, startRow = 8) 
...
ID <- "P668"; season <- "26F"; type <- "CA-Clement"; warehouse <- "WH-SURREY"
file <- list.files(path = paste0("../PO/order/", season, "/"), pattern = paste0(ID, ".*.xlsx"), full.names = TRUE, recursive = T)
...
# ------------ upload CEFA POs -------------------
ID <- "CEFA26F"; season <- "26F"; ReceiveDate <- "8/20/2026"
netsuite_item <- read.csv(rownames(file.info(list.files(path = "../NetSuite/", pattern = "Items_All_", full.names = TRUE)) %>% filter(mtime == max(mtime))), as.is = T)
PO <- read_xlsx(rownames(file.info(list.files(path = "../PO/order/CEFA/", pattern = ".xlsx", full.names = TRUE)) %>% filter(mtime == max(mtime))), sheet = 1, startRow = 2, check_names = T) %>% 
...
library(tidyr)
season <- "26F"; warehouse <- "AWD-US"; PO_suffix <- "-CA"
RefNo <- "26FWUS11"; ShippingDate <- "7/16/2026"; ReceiveDate <- "8/16/2026"; AMZ.Shipment.ID <- "STAR-QZSQPKYMXEMBC"
season <- "26F"; warehouse <- "WH-CHINA"; PO_suffix <- "-JP"
RefNo <- "26FWJP1"; ShippingDate <- "8/31/2026"; ReceiveDate <- "8/31/2026"; AMZ.Shipment.ID <- ""
...
for(s in 2:length(sheets)){
  ...
  shipment_i <- read_xlsx(shipment_in, sheet = sheet, startRow = 5, fillMergedCells = T, check_names = T) %>% rename_with(~ gsub("PO.*", "PO", .)) %>% mutate(across(where(is.character), trimws)) %>% 
    select(1:PER.CTN) %>% filter(grepl(".*\\-.*", SKU)) %>% tidyr::fill(1:Season, .direction = "down") %>% mutate(Unit.weight = ifelse(as.numeric(`Unit.Weight.g.`) < 10, round(as.numeric(`Unit.Weight.g.`)*1000, 0), round(as.numeric(`Unit.Weight.g.`), 0))) 
...
shipment <- shipment %>% mutate(PO.REF.NO = ifelse(PO == "PO#NA", gsub(PO_suffix, "", PO.REF.NO), PO.REF.NO), PO = paste0("PO#", POn[PO.REF.NO, "Document.Number"]))
...
if(nrow(OverReceive)){
  p <- "PO#PO000785" # over-receiving PO 
...
RefNo <- "26FWCA14"; ShippingDate <- "8/17/2026"; ReceiveDate <- "9/17/2026"
...
for(s in 2:length(sheets)){
  ...
  Nbox <- Nbox + length(unique(shipment_o$BOX.NO))
  print(paste0("Total Qty ", sum(shipment_o$QUANTITY), "; Total Box# ", length(unique(shipment_o$BOX.NO))))
  if(grepl(TO_prefix, sheets[s])){
    TO <- rbind(TO, data.frame(Date = format(Sys.Date(), "%m/%d/%Y"), TO.TYPE = "China-Surrey", SEASON = season, FROM.WAREHOUSE = "WH-CHINA", TO.WAREHOUSE = "WH-SURREY", REF.NO = TO_Ref, Memo = RefNo, ORDER.PLACED.BY = "Gloria Li", ITEM = shipment_o$ITEM, Quantity = shipment_o$QUANTITY))
  }else{
    shipment <- rbind(shipment, shipment_o %>% select(REF.NO:PO))
  }
  attachment <- rbind(attachment, data.frame(Sheet.Number = c(gsub("-.*", "", shipment_o$BOX.NO), ""), Box.Number = c(gsub(".*-", "", shipment_o$BOX.NO), ""), SKU = c(shipment_o$ITEM, ""), Packing.Slip.QTY = c(shipment_o$QUANTITY, ""), Notes = ""))
}
if(nrow(TO)){
...
if(nrow(TO)){
  ...
  netsuite_item[netsuite_item == "" | is.na(netsuite_item)] <- 0
  TO <- TO %>% group_by(ITEM) %>% mutate(Quantity = sum(Quantity), ITEM = gsub("-V1", "", ITEM)) %>% distinct(ITEM, .keep_all = T) %>% ungroup() %>% mutate(Qty.availeble = ifelse(ITEM %in% netsuite_item$Name, netsuite_item[ITEM, "Warehouse.Available"], 0), Qty.add = Quantity - Qty.availeble)
...
shipment <- shipment %>% group_by(PO.REF.NO, ITEM) %>% mutate(QUANTITY = sum(QUANTITY), ITEM = gsub("-V1", "", ITEM), BOX.NO = paste0(BOX.NO, collapse = " | ")) %>% distinct(PO.REF.NO, ITEM, .keep_all = T) %>% ungroup()
OverReceive <- shipment %>% mutate(Qty.Remain = PO_detail[paste0(PO.REF.NO, "_", ITEM), "Quantity.Remain"], Qty.Remain = ifelse(is.na(Qty.Remain), 0, Qty.Remain), Qty = QUANTITY - Qty.Remain) %>% filter(QUANTITY > Qty.Remain)
if(nrow(OverReceive)){
  View(OverReceive)
  PO_OverReceive <- OverReceive %>% mutate(PO.TYPE = "Over Received", CATEGORY = "MIX", SEASON = season, REF.NO = paste0("Over.", RefNo), WAREHOUSE = gsub("FBA", "WH-AMZ : FBA", warehouse), VENDOR = "China", CURRENCY = "CAD", ORDER.DATE = format(Sys.Date(), "%m/%d/%Y"), ORDER.PLACED.BY = "Gloria Li", APPROVAL.STATUS = "APPROVED", DUE.DATE = ReceiveDate, MEMO = paste0("Over receiving in shipment ", RefNo), ITEM. = ITEM, QUANTITY. = Qty, Tax.Code = "CA-Zero", External.ID = REF.NO) %>% 
...
# output for NS: upload shipment to Inbound Shipment and attach attachment in Communication tab
if(nrow(OverReceive)){
  p <- "PO#PO000778" # over-receiving PO 
  shipment <- shipment %>% mutate(Qty.Remain = PO_detail[paste0(PO.REF.NO, "_", ITEM), "Quantity.Remain"], Qty.Remain = ifelse(is.na(Qty.Remain), 0, Qty.Remain), QUANTITY = ifelse(QUANTITY > Qty.Remain, Qty.Remain, QUANTITY)) %>% filter(PO != "PO#NA") %>% select(-Qty.Remain)
  shipment <- rbind(shipment, data.frame(REF.NO = RefNo, EXPECTED.SHIPPING.DATE = ShippingDate, EXPECTED.DELIVERY.DATE = ReceiveDate, MEMO = memo, PO.REF.NO = paste0("Over.", RefNo), BOX.NO = OverReceive$BOX.NO, ITEM = OverReceive$ITEM, QUANTITY = OverReceive$Qty, LOCATION = warehouse, PO = p)) %>% filter(QUANTITY != 0) %>% 
    mutate(BOX.NO = ifelse(nchar(BOX.NO) > 300, substring(BOX.NO, 1, 299), BOX.NO)) %>% arrange(ITEM) 
}
...
shipment_weight <- read_xlsx(list.files(path = "../PO/shipment/", pattern = paste0("BoxWeight_", RefNo, ".xlsx"), recursive = T, full.names = T)) %>% 
