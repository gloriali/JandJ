...
products_upload <- products_XHS %>% mutate(Inventory = ifelse(netsuite_item_S[toupper(SKU), "Warehouse.Available"] < 10, 0, netsuite_item_S[toupper(SKU), "Warehouse.Available"]), Price = ifelse(grepl("^L", SKU), woo[toupper(SKU), "Sale.price"] + 10, woo[toupper(SKU), "Sale.price"]), Compare.At.Price = ifelse(grepl("^L", SKU), woo[toupper(SKU), "Regular.price"] + 10, woo[toupper(SKU), "Regular.price"]), Tags = ifelse(Price == Compare.At.Price, "正价", "特价"))
...
RefNo <- "25SSDE6"; ShippingDate <- "4/2/2024"; ReceiveDate <- "4/30/2025"
...
for(s in 2:length(sheets)){
  ...
  shipment_i <- openxlsx::read.xlsx(shipment_in, sheet = sheet, startRow = 5, fillMergedCells = T) %>% 
    select(1:Season) %>% filter(grepl(".*\\-.*", SKU)) %>% tidyr::fill(1:Season, .direction = "down") %>% mutate(`PO.#` = str_trim(`PO.#`))
...
if(nrow(OverReceive)){
  p <- "PO#PO000246" # over-receiving PO 
