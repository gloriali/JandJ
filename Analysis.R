...
warehouse_sales_new <- data.frame(Type = "Discontinued", SKU = c((xoro %>% filter(!grepl("24", Seasons), grepl("^WJA-", Item.), ATS > 0))$Item., (xoro %>% filter(!grepl("24", Seasons), grepl("^WPS-", Item.), ATS > 0))$Item., (xoro %>% filter(!grepl("24", Seasons), grepl("^WSF-", Item.), ATS > 0))$Item.)) %>% 
  mutate(Sales.price = ifelse(grepl("WJA", SKU), 50, 25), Sales.price = ifelse(grepl("WSF", SKU), 60, Sales.price), Surrey_Qty = xoro[SKU, "ATS"], Qty_transfer = 0)
warehouse_sales <- rbind(warehouse_sales, warehouse_sales_new)
...
clover_item <- openxlsx::readWorkbook(clover, "Items") %>% 
  mutate(Price = ifelse(Name %in% warehouse_sales$SKU, warehouse_sales[Name, "Sales.price"], Price), Price = ifelse(Name %in% price_deal$Name, price_deal[Name, "Price"], Price), Price = ifelse(grepl("^WPS", Name), "25.00", Price), Price = ifelse(grepl("^WJA", Name), "50.00", Price), Price = ifelse(grepl("^WMT", Name), "15.00", Price), Price = ifelse(grepl("SSS", Name), "10.00", Price), Price = ifelse(grepl("SSW", Name), "10.00", Price), Price = ifelse(grepl("BTB", Name), "40.00", Price), Price = ifelse(grepl("BTP", Name), "40.00", Price), Price = ifelse(grepl("WSS-CNL", Name), "40.00", Price), Price = ifelse(grepl("WSS-DNL", Name), "40.00", Price), Price = ifelse(grepl("WSS-TRZ", Name), "40.00", Price), Price = ifelse(grepl("WSS-UNC", Name), "40.00", Price), Price = ifelse(grepl("ICP", Name), "60.00", Price), Price.Type = ifelse(is.na(Price), "Variable", "Fixed")) %>% `row.names<-`(.[, "Name"])
...
order <- data.frame(StoreCode = "WH-JJ", ItemNumber = (clover_item %>% filter(Name %in% warehouse_sales$SKU, Quantity < 10))$Name, Qty = 20 - clover_item[(clover_item %>% filter(Name %in% warehouse_sales$SKU, Quantity < 10))$Name, "Quantity"], LocationName = "BIN", UnitCost = "", ReasonCode = "RWT", Memo = "Richmond Transfer to Miranda", UploadRule = "D", AdjAccntName = "", TxnDate = "", ItemIdentifierCode = "", ImportError = "") %>% 
  mutate(ATS = xoro[ItemNumber, "ATS"], cat = gsub("-.*", "", ItemNumber), size = gsub("\\w+-\\w+-", "", ItemNumber), Qty = ifelse(ATS < 10, ATS, Qty)) %>% filter(ATS > 0) %>% arrange(cat, size) %>% select(-c("cat", "size"))
