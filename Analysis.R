...
# setup Clover
sales <- openxlsx::read.xlsx("../Analysis/WarehouseSale_20250708.xlsx", sheet = 1) %>% `row.names<-`(.[, "Name"])
price <- openxlsx::read.xlsx("../Analysis/WarehouseSale_20250708.xlsx", sheet = 2) %>% `row.names<-`(.[, "Cat"])
clover <- openxlsx::loadWorkbook(list.files(path = "../Clover/", pattern = paste0("inventory", format(Sys.Date(), "%Y%m%d"), ".xlsx"), full.names = T))
clover_item <- openxlsx::readWorkbook(clover, "Items") %>% filter(Name != "") %>% 
  mutate(Price = ifelse(Name %in% sales$Name, sales[Name, "Price"], Price), Quantity = ifelse(Name %in% netsuite_item_S$Name, netsuite_item_S[Name, "Warehouse.Available"], 0), Cat = gsub("-.*", "", Name))
clover_item <- clover_item %>% mutate(Price = ifelse(!(Name %in% sales$Name) & Cat %in% price$Cat, price[Cat, "Price"], Price))
clover_item <- clover_item %>% mutate(Price = ifelse(grepl("^HJS", Name), 10, Price))
clover_item <- clover_item %>% mutate(Price = ifelse(grepl("^MWPS-YEL-10Y", Name), 10, Price))
clover_item <- clover_item %>% mutate(Price.Type = ifelse(is.na(Price), "Variable", "Fixed")) %>% select(-Cat)
clover_item <- clover_item %>% rename_with(~ gsub("\\.", " ", colnames(clover_item)))
deleteData(clover, sheet = "Items", cols = 1:ncol(clover_item), rows = 1:nrow(clover_item) + 100, gridExpand = T)
writeData(clover, sheet = "Items", clover_item)
openxlsx::saveWorkbook(clover, file = paste0("../Clover/inventory", format(Sys.Date(), "%Y%m%d"), "-upload.xlsx"), overwrite = T)
# upload SO to NS
payments <- read.csv(rownames(file.info(list.files(path = "../Clover/", pattern = "Payments-", full.names = TRUE)) %>% filter(mtime == max(mtime))), as.is = T) %>% filter(Result == "SUCCESS") %>% `row.names<-`(.[, "Order.ID"]) 
clover_so <- read.csv(rownames(file.info(list.files(path = "../Clover/", pattern = "LineItemsExport-", full.names = TRUE)) %>% filter(mtime == max(mtime))), as.is = T) %>% mutate(Item.SKU = ifelse(Item.SKU %in% netsuite_item_S$Name, Item.SKU, "MISC-ITEM"), Tender = payments[Order.ID, "Tender"])
netsuite_so <- clover_so %>% filter(Item.SKU != "", Order.Payment.State == "Paid", !grepl("XHS", Order.Discounts), !grepl("Surrey", Order.Discounts)) %>% mutate(Order.date = format(as.Date(gsub(" .*", "", Line.Item.Date), "%d-%b-%Y"), "%m/%d/%Y")) %>% group_by(Order.date) %>% 
  mutate(Payment.Option = ifelse(Tender == "Cash", "Cash", ifelse(grepl("WeChat", Tender), "AlphaPay", "Clover")), Class = "FBM : CA", MEMO = "Warehouse sales", Customer = "55 JJR SHOPS", ID = data.table::rleid(Order.ID), REF.ID = paste0("CL", format(as.Date(Order.date, "%m/%d/%Y"), "%y%m%d"), "-", sprintf("%02d", ID)), Order.Type = "JJR SHOPS", Department = "Retail : Store Surrey", Warehouse = "WH-SURREY", Quantity = 1, Price.level = "Custom", Rate = Item.Total, Coupon.Discount = Total.Discount + Order.Discount.Proportion, Coupon.Discount = ifelse(Coupon.Discount == 0, NA, Coupon.Discount), Coupon.Code = gsub("NA", "", gsub(" -.*","", paste0(Order.Discounts, Discounts))), Tax.Code = ifelse(Item.Tax.Rate == 0.05, "CA-BC-GST", ifelse(Item.Tax.Rate == 0.12, "CA-BC-TAX", "")), SHIPPING.CARRIER = "", SHIPPING.METHOD = "", SHIPPING.COST = 0, Recipient = "", Recipient.Phone = "", Recipient.Email = "") %>% 
...
errors <- read.csv("../Clover/results.csv", as.is = T) %>% filter(!(Item.SKU %in% (netsuite_item_S %>% filter(Warehouse.Available > 0))$Name))
write.csv(errors, file = paste0("../Clover/SO-errors-", format(Sys.Date(), "%Y%m%d"), ".csv"), row.names = F, na = "")
# return Clover to RI
woo <- read.csv(list.files(path = "../woo/", pattern = gsub("-0", "-", paste0("wc-product-export-", format(Sys.Date(), "%d-%m-%Y"))), full.names = T), as.is = T) %>% 
...
before_sale <- openxlsx::read.xlsx("../Clover/inventory20250712.xlsx", sheet = 1) %>% filter(Name != "") %>% `row.names<-`(.[, "Name"])
price <- woo %>% mutate(cat = gsub("-.*", "", SKU)) %>% group_by(cat) %>% summarise(Price = max(Sale.price)) %>% as.data.frame()
price <- rbind(price, data.frame(cat = c("MISC5", "MISC10", "MISC15", "MISC20", "DBRC", "DBTB", "DBTL", "DBTP", "DLBS", "DWJA", "DWJT", "DWPF", "DWPS", "DWSF", "DWSS", "DXBK", "MAJC"), Price = c(5, 10, 15, 20, 30, 35, 40, 40, 25, 50, 40, 30, 25, 60, 55, 30, 99.99))) %>% `row.names<-`(toupper(.[, "cat"])) 
...
clover <- openxlsx::loadWorkbook(list.files(path = "../Clover/", pattern = paste0("inventory", format(Sys.Date(), "%Y%m%d"), ".xlsx"), full.names = T))
clover_item <- openxlsx::readWorkbook(clover, "Items") %>% filter(Name != "") %>% 
  mutate(cat = gsub("-.*", "", Name), Quantity = before_sale[Name, "Quantity"], Price = ifelse(Name %in% woo$SKU, woo[Name, "Sale.price"], price[cat, "Price"]), Price.Type = ifelse(is.na(Price), "Variable", "Fixed"), Alternate.Name = woo[Name, "Name"], Tax.Rates = ifelse(woo[Name, "Tax.class"] == "full", "GST+PST", "GST"), Tax.Rates = ifelse(is.na(Tax.Rates), "GST", Tax.Rates)) %>% select(-cat)
...
clover <- openxlsx::loadWorkbook(list.files(path = "../Clover/", pattern = paste0("inventory", format(Sys.Date(), "%Y%m%d"), ".xlsx"), full.names = T))
clover_item <- readWorkbook(clover, "Items") %>% filter(Name != "") %>% 
  mutate(cat = gsub("-.*", "", Name), Price = ifelse(Name %in% woo$SKU, woo[Name, "Regular.price"], price[cat, "Price"]), Alternate.Name = woo[Name, "Name"], Tax.Rates = ifelse(woo[Name, "Tax.class"] == "full", "GST+PST", "GST"), Tax.Rates = ifelse(is.na(Tax.Rates), "GST", Tax.Rates))
clover_item <- clover_item %>% mutate(Price = ifelse(grepl("^SS", Name), 10, Price))
clover_item <- clover_item %>% mutate(Price = ifelse(grepl("^WHO", Name), 27, Price))
clover_item <- clover_item %>% mutate(Price = ifelse(grepl("^LCP", Name), 28, Price))
clover_item <- clover_item %>% mutate(Price = ifelse(grepl("^LCT", Name), 32, Price))
clover_item <- clover_item %>% mutate(Price.Type = ifelse(is.na(Price), "Variable", "Fixed")) %>% select(-cat)
clover_item <- clover_item %>% rename_with(~ gsub("\\.", " ", colnames(clover_item)))
deleteData(clover, sheet = "Items", cols = 1:ncol(clover_item), rows = 1:nrow(clover_item) + 100, gridExpand = T)
writeData(clover, sheet = "Items", clover_item)
...
woo <- read.csv(rownames(file.info(list.files(path = "../woo/", pattern = "wc-product-export-", full.names = T)) %>% filter(mtime == max(mtime))), as.is = T) %>% 
  filter(!is.na(Regular.price) & !duplicated(SKU) & SKU != "") %>% mutate(Sale.price = ifelse(is.na(Sale.price), Regular.price, Sale.price)) %>% `row.names<-`(.[, "SKU"])
price <- woo %>% mutate(cat = gsub("-.*", "", SKU)) %>% group_by(cat) %>% summarise(Price = max(Sale.price)) %>% as.data.frame()
price <- rbind(price, data.frame(cat = c("MISC5", "MISC10", "MISC15", "MISC20", "MISC25", "MISC30", "MISC35", "MISC45"), Price = c(5, 10, 15, 20, 25, 30, 35, 45))) %>% `row.names<-`(toupper(.[, "cat"])) 
items <- read.csv("../Analysis/WarehouseSale_2026-05-20.csv", as.is = T) %>% 
  mutate(Qty = ifelse(netsuite_item_S[SKU, "Warehouse.Available"] == 0, Qty, netsuite_item_S[SKU, "Warehouse.Available"]), Seasons = netsuite_item_S[SKU, "Item.SKU.Seasons"], Print = netsuite_item_S[SKU, "Item.Print.SKU"], Size = netsuite_item_S[SKU, "Item.Size"], Name = woo[SKU, "Name"], Regular.price = woo[SKU, "Regular.price"], Sale.price = woo[SKU, "Sale.price"])
write.csv(items, file ="../Analysis/WarehouseSale_2026-05-20.csv", row.names = F)
## Update warehouse sale price
clearance <- read.csv("../Analysis/WarehouseSale_2026-05-20.csv", as.is = T) %>% select(Item, Sales)
...
clover_so <- read.csv(rownames(file.info(list.files(path = "../Clover/", pattern = "LineItemsExport-", full.names = TRUE)) %>% filter(mtime == max(mtime))), as.is = T) %>% mutate(Item.SKU = ifelse(grepl("UT1-LGN", Item.SKU), "MISC-ITEM", Item.SKU), Item.SKU = ifelse(grepl("SBS", Item.SKU), "MISC-ITEM", Item.SKU), Item.SKU = ifelse(Item.SKU %in% netsuite_item$Name, Item.SKU, "MISC-ITEM"), Recipient = payments[Order.ID, "Customer.Name"], Recipient.Phone = payments[Order.ID, "Phone"], Recipient.Email = payments[Order.ID, "Email"], Tender = payments[Order.ID, "Tender"])
netsuite_so <- clover_so %>% filter(Item.SKU != "", Order.Payment.State == "Paid", Refunded == "false", Exchanged == "false", !grepl("XHS", Order.Discounts), !grepl("Surrey", Order.Discounts)) %>% mutate(Order.date = format(as.Date(gsub(" .*", "", Line.Item.Date), "%d-%b-%Y"), "%m/%d/%Y")) %>% group_by(Order.date) %>% 
  mutate(Payment.Option = ifelse(Tender == "Cash", "Cash", ifelse(grepl("WeChat", Tender), "AlphaPay", "Clover")), Class = "FBM : CA", MEMO = "Warehouse sales 2026", Customer = "55 JJR SHOPS", ID = data.table::rleid(Order.ID), REF.ID = paste0("CLS", format(as.Date(Order.date, "%m/%d/%Y"), "%y%m%d"), "-", sprintf("%02d", ID)), Order.Type = "JJR SHOPS", Department = "Retail : Store Surrey", Warehouse = "WH-SURREY", Quantity = 1, Price.level = "Custom", Rate = Item.Total, Coupon.Discount = Total.Discount + Order.Discount.Proportion, Coupon.Discount = ifelse(Coupon.Discount == 0, NA, Coupon.Discount), Coupon.Code = gsub("NA", "", gsub(" -.*","", paste0(Order.Discounts, Discounts))), Tax.Code = ifelse(Item.Tax.Rate == 0.05, "CA-BC-GST", ifelse(Item.Tax.Rate == 0.12, "CA-BC-TAX", "")), SHIPPING.CARRIER = "", SHIPPING.METHOD = "", SHIPPING.COST = 0) %>% 
  select(Payment.Option, Class, Order.date, REF.ID, Order.Type, Department, Warehouse, MEMO, Order.ID, Item.SKU, Quantity, Price.level, Rate, Coupon.Discount, Coupon.Code, Tax.Code, Tax.Amount, Customer, Recipient, Recipient.Phone, Recipient.Email, SHIPPING.CARRIER, SHIPPING.METHOD, SHIPPING.COST) %>% filter(!is.na(Payment.Option))
write.csv(netsuite_so, file = paste0("../Clover/SO-clover-", format(Sys.Date(), "%Y%m%d"), ".csv"), row.names = F, na = "")
### SO errors
...
price <- woo %>% mutate(cat = gsub("-.*", "", SKU)) %>% group_by(cat) %>% summarise(Price = max(Regular.price, na.rm = T)) %>% as.data.frame()
price <- rbind(price, data.frame(cat = c("MISC5", "MISC10", "MISC15", "MISC20", "MISC25", "MISC30", "MISC35", "MISC45", "DBRC", "DBTB", "DBTL", "DBTP", "DLBS", "DWJA", "DWJT", "DWPF", "DWPS", "DWSF", "DWSS", "DXBK", "MAJC"), Price = c(5, 10, 15, 20, 25, 30, 35, 45, 30, 35, 40, 40, 25, 50, 40, 30, 25, 60, 55, 30, 99.99))) %>% `row.names<-`(toupper(.[, "cat"])) 
clover <- wb_load(list.files(path = "../Clover/", pattern = paste0("inventory", format(Sys.Date(), "%Y%m%d"), ".xlsx"), full.names = T)) |> wb_remove_worksheet(sheet = 1) 
clover_item <- wb_to_df(clover, "Items") %>% filter(Name != "") %>% mutate(cat = gsub("-.*", "", Name), Price = ifelse(Name %in% woo$SKU, round(woo[Name, "Regular.price"]*0.85, 2), round(price[cat, "Price"]*0.85, 2)), Price = ifelse((Name %in% woo$SKU) & (Price > woo[Name, "Sale.price"]), woo[Name, "Sale.price"], Price), `Price Type` = ifelse(is.na(Price), "Variable", "Fixed"), `Alternate Name` = woo[Name, "Name"], `Tax Rates` = ifelse(cat %in% PST, "GST+PST", "GST")) %>% select(-cat)
clover_item <- clover_item %>% regex_left_join(clearance, by = c("Name" = "Item")) %>% mutate(Price = coalesce(Sales, Price), `Price Type` = ifelse(is.na(Price), "Variable", "Fixed")) %>% select(all_of(names(clover_item))) %>% distinct(Name, .keep_all = T)
clover_update <- wb_workbook()
for(s in clover$get_sheet_names()){clover_update <- clover_update |> wb_add_worksheet(sheet = s) |> wb_add_data(sheet = s, x = wb_to_df(clover, s))}
clover_update <- clover_update |> wb_clean_sheet(sheet = "Items") |> wb_add_data(sheet = "Items", x = clover_item) |> wb_add_numfmt(sheet = "Tax Rates", dims = "C2:C3", numfmt = "0%")
